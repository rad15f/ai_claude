#!/usr/bin/env python3
"""
Generate ~1,500 bot-like YouTube comments using Gemini Flash (Google).
Output: dataset/bot_comments_gemini.csv

Usage:
    pip install google-genai
    GEMINI_API_KEY=AQ... python3 training/generate_bot_comments_gemini.py

Get a free key at: https://aistudio.google.com/app/apikey
Free tier: 15 requests/min, 1,500 requests/day — enough for this script.
"""
...
OUTPUT_FILE  = OUTPUT_DIR / "bot_comments_gemini.csv"
...
def call_with_retry(client: "genai.Client", topic: str, max_retries: int = 3) -> list:
    for attempt in range(max_retries):
        try:
            ...
            err = str(e)
            err_lower = err.lower()
            print(f"  [attempt {attempt + 1}/{max_retries}] RAW ERROR: {err}")
            if "api_key_invalid" in err_lower or "api key not valid" in err_lower or "permission_denied" in err_lower:
                sys.exit(f"ERROR: Invalid GEMINI_API_KEY.\n{err}")
            elif "resource_exhausted" in err_lower or "429" in err or "quota" in err_lower:
                wait = 60 + random.uniform(0, 10)
                print(f"  Rate limit — waiting {wait:.0f}s (free tier: 15 req/min)")
                if attempt < max_retries - 1:
                    time.sleep(wait)
            else:
                wait = 2 ** attempt + random.uniform(0, 1)
...
def main() -> None:
    ...
    if not api_key:
        sys.exit("Set GEMINI_API_KEY environment variable. Get a free key at https://aistudio.google.com/app/apikey")
    ...
    for batch_idx in range(start_batch, TOTAL_BATCHES):
        ...
        # Free tier: 15 req/min → 1 request every 4 seconds minimum
        time.sleep(4.5)
    ...
    print(f"\n✓ {len(unique_rows)} unique Gemini Flash bot comments → {OUTPUT_FILE}")
